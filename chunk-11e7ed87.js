...
`;var Qe=new D("BACKPACKING_RULES_TEMPLATE");function cs(){return Qt;}var Ye=new D("DEFAULT_RULES_TEMPLATE");function us(s){return s.startsWith("de")?Yt:Jt;}var Je=new D("EMPTY_RULES_TEMPLATE");function fs(s){return s.startsWith("de")?Zt:Xt;}var Ze=new D("LOGIC_RULES_TEMPLATE");function gs(){return er;}function Yn(){return At([{provide:Qe,useFactory:cs,deps:[me]},{provide:Ye,useFactory:us,deps:[me]},{provide:Je,useFactory:fs,deps:[me]},{provide:Ze,useFactory:gs,deps:[me]}]);}var tr=new D("CAPACITOR_HTTP_REQUEST_MODE");function ps(s){switch(s){case 400:return"Ung\xFCltige Anfrage";case 401:return"Nicht autorisiert";case 403:return"Verboten";case 404:return"Nicht gefunden";case 500:return"interner Serverfehler";case 504:return"Zeit\xFCberschreitung";default:return;}}var rr=({config:{rulesTemplate:s}})=>{let o=G("rulesMode","template"),e=G("rules",void 0),a=x(()=>e()!==void 0),c=x(()=>{let m=e();if(m)return hs(m).toString(16);}),p=G("lastExportHash",void 0),h=G("lastExportDate",void 0),T=()=>{p.set(c()),h.set(new Date().getTime());},v=N(new Date().getTime());L(()=>{e(),v.set(new Date().getTime());});let H=y(Ye),U=y(Je),W=y(Ze),b=y(Qe),$=x(()=>{switch(s()){case"empty":return U;case"logic":return W;case"backpacking":return b;default:return H;}}),I=G("remoteHistory",[]),F=G("remoteHistoryTitle",{}),oe=function(m){I.update(O=>O.filter(C=>C!==m)),F.update(O=>{let C=X({},O);return delete C[m],C;});},Z=ke(()=>o()==="remote"?I()[0]:void 0),ae=y(tr,{optional:!0})??"cors",z=ve({request:()=>({mode:o(),rawLocal:e(),remote:Z(),template:$()}),loader:({request:m})=>{switch(m.mode){case"local":return Promise.resolve(m.rawLocal);case"template":return Promise.resolve(m.template);case"remote":if(m.remote){if(!m.remote.startsWith("http"))throw new Error("Invalid URL");let O=qt(m.remote,ae==="cors");return kt.get({url:O,webFetchExtra:{mode:ae}}).then(C=>{if(C.status>=200&&C.status<300)return I.update(A=>[m.remote,...A.filter(R=>R!==m.remote)]),C.data;{let A=[["HTTP Fehler",C.status.toString()].join(" "),ps(C.status)].join(": ");throw new Error(A);}});}else return Promise.resolve(void 0);}}}),le=function(m){Z()!==m&&(o.set("remote"),Z.set(m));},ce=function(m){let O=Z();o()==="remote"&&O&&F.update(C=>Ae(X({},C),{[O]:m}));},ue=function(m){o.set("local"),e.set(m);},w=function(){e.set(z.value()),o.set("local"),T();};return{rules:{mode:o,raw:z.asReadonly(),lastAction:v.asReadonly(),hash:c,exportNeeded:x(()=>o()==="local"&&c()!==p()),markAsCurrent:T,localRulesAvailable:a},export:{lastDate:h.asReadonly()},localRules:{updateRules:ue,copyFromCurrent:w},remoteRules:{load:le,setCurrentTitle:ce,history:x(()=>I().map(m=>({url:m,title:F()[m]}))),removeFromHistory:oe}};},hs=function(s,o=0){let e=3735928559^o,a=1103547991^o;for(let c=0,p;c<s.length;c++)p=s.charCodeAt(c),e=Math.imul(e^p,2654435761),a=Math.imul(a^p,1597334677);return e=Math.imul(e^e>>>16,2246822507),e^=Math.imul(a^a>>>13,3266489909),a=Math.imul(a^a>>>16,2246822507),a^=Math.imul(e^e>>>13,3266489909),4294967296*(2097151&a)+(e>>>0);};var sr=()=>({serviceWorker:{status:N("init")}});function ds(){return _e.builder().extend(Ft).extend(Mt).extend(sr).extend(Pt).extend(Dt).extend(rr).extend(jt).extend(Ut);}var nr=new D("STATE_BUILDER",{providedIn:"root",factory:()=>ds()}),ms=new D("GLOBAL_STATE",{providedIn:"root",factory:()=>y(nr).build()}),ws=new D("RESET_SWITCH",{providedIn:"root",factory:()=>{let s=y(nr);return async()=>{await s.reset();};}});export{ys as a,Ie as b,Ts as c,ee as d,Y as e,J as f,q as g,xs as h,we as i,te as j,re as k,se as l,ne as m,ts as n,Le as o,Be as p,Yn as q,tr as r,ms as s,ws as t};/**i18n:28a3168b76bee7dffc4b1e816012f24aa421b4da90b9b896cc67ce89fdf6fb94*/